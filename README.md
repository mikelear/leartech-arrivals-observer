...
| Logging | Structured JSON (`LOG_FORMAT=json` in-cluster) with `service`/`version`/`cluster` ambient fields; every arrival lifecycle transition emits `event=arrival_created|arrival_pending|arrival_testing|pack_dispatched|pack_result|arrival_passed|arrival_failed|arrival_timeout` so a Loki query like `{app="leartech-arrivals-observer"} \| json \| arrival="<x>" \| event=~"arrival_.*\|pack_.*"` traces one arrival end-to-end. | `internal/logging/logging.go`, `internal/controller/controller.go` |
| Metrics | `/metrics` (Prometheus), scoped via NetworkPolicy. Custom counters + histogram: `arrivals_observer_arrival_finalized_total{phase,service}`, `arrivals_observer_pack_duration_seconds{service,pack}` (histogram), `arrivals_observer_pack_result_total{status,service}`, `arrivals_observer_job_oom_total{service}`. | `internal/handlers/metrics.go`, `internal/metrics/metrics.go` |
| Dashboard | Grafana dashboard as ConfigMap — Loki-backed (arrival lifecycle, pack logs) + Prometheus panels (pass/fail rate, pack p99, OOM counts). Discovered by the JX Grafana sidecar via `jenkins-x.io/grafana-dashboard: "1"` label; folder set via `grafana_folder` annotation. Same shape as the automated-agent dashboards. | `charts/leartech-arrivals-observer/templates/grafana-dashboard.yaml`, `charts/leartech-arrivals-observer/files/grafana/arrivals-observer-dashboard.json` |
| Pack-job logs | Every dispatched pack Job tees its stdout+stderr to `/tmp/logs.jsonl` and uploads it to `gs://<bucket>/results/v1/post-deploy/<cluster>/<ns>/<service>/<version>/<pack>/logs.jsonl` before exit — so pack failures stay debuggable long after K8s reaps the pod. | `internal/dispatch/dispatch.go` (`runnerScript`) |
...
| Chart | uses `leartech-helm-library` for labels/securityContext/probes — consistent spine | `charts/leartech-arrivals-observer/` |
...
git clone --depth=1 https://github.com/mikelear/leartech-arrivals-observer.git <new-service-name>
cd <new-service-name>
rm -rf .git && git init && git branch -m main

# 3. Rename the module path (this is the one-shot manual step)
OLD=github.com/mikelear/leartech-arrivals-observer
...
git add -A && git commit -m "feat: clone from leartech-arrivals-observer"
git tag v0.0.1
